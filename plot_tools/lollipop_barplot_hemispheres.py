...
class HemisphereDifferencePlotter:
    ...
    def add_hemispheres_and_plot(self, files, dataset_names):
        ...
        ax.set_xlabel("Dataset", fontsize="large", weight='bold', labelpad=10)
        ax.set_ylabel("Percentage (%) of tumors in hemisphere", fontsize="large", weight='bold', labelpad=10)
        legend_props = {'weight': 'bold'}
        ax.legend(
            fontsize="large",
            prop=legend_props
        )
