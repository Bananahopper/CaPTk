...
class ConnectomeRegistrator:
    ...
    def run(self, source: str, label: str):
        ...
        for idx, (file, label) in tqdm(enumerate(zip(file_list, label_list))):
            self.register(file, label)

    def register(self, source: str, label: str):

        registration_folder, affine_folder, warp_folder = self.create_folder_structure(
            source
        )

        patient_output_t1_affine = os.path.join(
            registration_folder, "_connectome_aff_reg_t1.nii.gz"
        )
        patient_output_label_affine = os.path.join(
            registration_folder, "_connectome_aff_reg_label.nii.gz"
        )

        patient_output_t1_warp = os.path.join(
            registration_folder, "_connectome_warp_reg_t1.nii.gz"
        )
        patient_output_label_warp = os.path.join(
            registration_folder, "_connectome_warp_reg_label.nii.gz"
        )
    ...
    def create_folder_structure(self, source: str):
        ...
        dataset_name = source.split("/")[2]
        subject_id = source.split("/")[-2]
        ...
        return registration_folder, affine_folder, warp_folder


def main(source, label, target_atlas):
    registrator = ConnectomeRegistrator(target_atlas)
    registrator.run(source, label)
...
if __name__ == "__main__":
    ...
    main(args.source, args.label, args.target_atlas)
